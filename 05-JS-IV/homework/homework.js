...
function tieneEmail(usuario) {
  ...
  // Tu código:
  if(('email' in usuario) && usuario.email !== null){
    return true;
  }
  return false;
}
...
function pasarUsuarioAPremium(usuarios) {
  ...
  for (let i = 0; i <usuarios.length; i++){
    aux = usuarios[i];
    for(let clave in aux){
      if(clave === 'esPremium' && aux[clave] !== true){
        aux[clave] = true;
      }
    }
    usuarios[i] = aux;
...
function agregarMetodoCalculoDescuento(producto) {
  ...
  var descuento = function() {
    ...
    var procentaje = this.porcentajeDeDescuento;
    var descuento = precio * procentaje;
    precio -= descuento;
    return precio;
  }
